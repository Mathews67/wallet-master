...
//Newly introduced type for Customers to pay bills for water, Electricity and School
 type BillType = {
        #Water;
        #Electricity;
        #School;
...
private var users : HashMap.HashMap<Text, User> = HashMap.HashMap<Text, User>(0, Text.equal, Text.hash);
private var accountNumberToUser : HashMap.HashMap<Text, Text> = HashMap.HashMap<Text, Text>(0, Text.equal, Text.hash);
private var emailToUser : HashMap.HashMap<Text, Text> = HashMap.HashMap<Text, Text>(0, Text.equal, Text.hash);
...
public shared func registerCustomer(fullName : Text, email : Text, password : Text) : async Result.Result<Text, Text> {
if (Text.size(fullName) == 0 or Text.size(email) == 0 or Text.size(password) < 8) {
return #err("Invalid input. Ensure all fields are filled and password is at least 8 characters long.");
};
...
users.put(accountNumber, newCustomer);
accountNumberToUser.put(accountNumber, accountNumber);
emailToUser.put(email, accountNumber);

#ok("Customer registered successfully. Account number: " # accountNumber)
};
};
};

public func login(email : Text, password : Text) : async Result.Result<(Text, Text), Text> {
switch (emailToUser.get(email)) {
...
#ok("User logged in successfully as " # debug_show (user.role), userId)
} else {
...
users := HashMap.fromIter(Iter.fromArray(usersEntries), 0, Text.equal, Text.hash);
accountNumberToUser := HashMap.fromIter(Iter.fromArray(accountNumberToUserEntries), 0, Text.equal, Text.hash);
emailToUser := HashMap.fromIter(Iter.fromArray(emailToUserEntries), 0, Text.equal, Text.hash);
...
//Function handling billing logics for Customers
